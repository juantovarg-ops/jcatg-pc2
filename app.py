...
B2_R_KEY_ID = st.secrets["b2"]["B2_R_KEY_ID"]
B2_R_APPLICATION_KEY = st.secrets["b2"]["B2_R_APPLICATION_KEY"]
B2_W_KEY_ID = st.secrets["b2"]["B2_W_KEY_ID"]
B2_W_APPLICATION_KEY = st.secrets["b2"]["B2_W_APPLICATION_KEY"]
...
def conectar_b2_lectura():
    ...
    api = b2.B2Api(info)
    api.authorize_account("production", B2_R_KEY_ID, B2_R_APPLICATION_KEY)
...
def conectar_b2_escritura():
    ...
    info = b2.InMemoryAccountInfo()
    api = b2.B2Api(info)
    api.authorize_account("production", B2_W_KEY_ID, B2_W_APPLICATION_KEY)
    return api.get_bucket_by_name(B2_BUCKET_NAME)
...
st.set_page_config(page_title="Chat PDF + Backblaze + MongoDB", page_icon="📚")
